...
export const listarActividades = async (req, res) => {
  try {
    const query =
      "SELECT actividades. *, actividades.fecha_actividad, actividades.descripcion, mantenimientos.tipo_mantenimiento, tecnicos.nombres AS nombres_tecnico, tecnicos.apellidos AS apellidos_tecnico, equipos.nombre_equipo, usuarios.nombres AS nombres_responsable, usuarios.apellidos AS apellidos_responsable                                                                                FROM actividades                                                                                       JOIN mantenimientos ON mantenimientos.id_mantenimiento = actividades.fk_mantenimiento                JOIN tecnicos ON tecnicos.id_tecnico = actividades.fk_tecnico                                     JOIN usuarios ON usuarios.id_usuario = mantenimientos.fk_user_responsable                         JOIN equipos ON equipos.id_equipo = mantenimientos.fk_equipo                                       ORDER BY actividades.id_actividad DESC";
    const [result] = await pool.query(query);

    const actividadessConFechaFormateada = result.map(actividad => ({
      ...actividad,
      fecha_actividad: new Date(actividad.fecha_actividad).toISOString().split('T')[0]
  }));

    if (result.length > 0) {
      // res.status(200).json(result);
      res.status(200).json(actividadessConFechaFormateada);
    } else {
...
export const registrarActividad = async (req, res) => {
  try {
    const { fecha_actividad, descripcion, fk_mantenimiento, fk_tecnico } =
      req.body;
    const query =
      "INSERT INTO actividades (fecha_actividad, descripcion, fk_mantenimiento, fk_tecnico) VALUES (?,?,?,?)";
    const [result] = await pool.query(query, [
      fecha_actividad,
      descripcion,
      fk_mantenimiento,
